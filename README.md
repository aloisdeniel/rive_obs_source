...
native video source. The file's state machine binds to the current OBS scene
through a view model, letting you build motion graphics that react to your
stream in real time.
...
The plugin watches the `.riv` file (and the JSON data file, if you've pointed
one at it) and re-imports them whenever their mtime changes. Just export
from Rive, switch back to OBS, see the result — no toggle needed.
