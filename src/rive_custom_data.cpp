...
void set_image_if_present(rive::ViewModelInstanceRuntime *vm, const char *name,
			  const char *url, ImageCache *cache)
{
	if (!vm)
		return;
...
std::string read_type_field(obs_data_t *obj) { return read_string_field(obj, kTypeKey); }
std::string read_id_field(obs_data_t *obj) { return read_string_field(obj, kIdKey); }
...
std::string resolve_list_item_vm_name(rive::ViewModelInstanceListRuntime *list,
...
std::unique_ptr<CustomDataBinding>
CustomDataBinding::tryCreate(rive::File *file, rive::Artboard *artboard,
			     rive::StateMachineInstance *sm,
			     rive::ViewModelInstanceRuntime *scene_root, ImageCache *images)
{
	if (!file)
		return nullptr;
	return std::unique_ptr<CustomDataBinding>(
		new CustomDataBinding(file, artboard, sm, scene_root, images));
}

CustomDataBinding::CustomDataBinding(rive::File *file, rive::Artboard *artboard,
				     rive::StateMachineInstance *sm,
				     rive::ViewModelInstanceRuntime *scene_root,
				     ImageCache *images)
	: m_file(file), m_artboard(artboard), m_sm(sm), m_images(images),
	  m_rootResolved(scene_root != nullptr), m_root(scene_root)
{
}
...
void CustomDataBinding::apply(obs_data_t *data)
{
	...
	if (!m_routingResolved) {
		...
		if (m_ownedRoot == nullptr && !requested.empty() &&
		    requested != m_root->viewModelName()) {
	...
	if (m_ownedRoot) {
		...
		if (!new_id.empty() && !m_rootId.empty() && new_id != m_rootId) {
			...
			rive::ViewModelRuntime *vm_runtime =
				m_file->viewModelByName(vm_type);
			if (vm_runtime) {
	...
	for (obs_data_item_t *it = obs_data_first(obj); it; obs_data_item_next(&it)) {
		...
		case OBS_DATA_NUMBER:
			if (obs_data_item_numtype(it) == OBS_DATA_NUM_INT) {
				set_number_if_present(
					vm, name,
					static_cast<double>(obs_data_item_get_int(it)));
			} else {
		...
		case OBS_DATA_OBJECT: {
			...
			rive::rcp<rive::ViewModelInstanceRuntime> nested =
				vm->propertyViewModel(name);
			...
			if (nested && child) {
				...
				// clean instance.
				if (!new_id.empty() && !slot.first.empty() &&
				    new_id != slot.first) {
					...
					if (vm_runtime) {
						rive::rcp<rive::ViewModelInstanceRuntime>
							fresh = vm_runtime->createInstance();
						if (fresh && vm->replaceViewModel(name,
										  fresh.get())) {
		...
		case OBS_DATA_ARRAY: {
			...
			if (list && arr) {
				...
				std::vector<rive::rcp<rive::ViewModelInstanceRuntime>>
					existing_no_id;
				...
				std::unordered_map<rive::ViewModelInstanceRuntime *,
						   std::string>
					ptr_to_id;
				...
				for (size_t i = 0; i < desired_count; ++i) {
					...
					if (!item) {
						...
						std::string item_vm =
							read_type_field(elem);
						if (item_vm.empty())
							item_vm = resolve_list_item_vm_name(
								list, arr);
						if (item_vm.empty()) {
							obs_log(LOG_WARNING,
							...
							rive::ViewModelRuntime *iv =
								m_file->viewModelByName(
									item_vm);
							if (!iv) {
								obs_log(LOG_WARNING,
									"rive: custom data "
				...
				bool sequence_changed =
					list->size() != desired_items.size();
				if (!sequence_changed) {
					for (size_t i = 0; i < desired_items.size();
					     ++i) {
						auto cur = list->instanceAt(
							static_cast<int>(i));
						if (cur.get() !=
						    desired_items[i].get()) {
