...
namespace {
                      ...
                      const std::string &sm_name, char *err, size_t err_size)
    {
        ...
        rive::rcp<rive::File> file =
            rive::File::import(rive::Span<const uint8_t>(bytes.data(), bytes.size()), r->renderContext.get(), &result);
        ...
        if (auto *defaultVm = r->file->defaultArtboardViewModel(r->artboard.get())) {
            obs_log(LOG_INFO, "rive: artboard primary view model = '%s'", defaultVm->name().c_str());
        } else {
